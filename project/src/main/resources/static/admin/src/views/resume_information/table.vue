<template>
	...
	<el-row class="rows row2">
		...
		<el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap search_btns">
			...
			<el-table-column label="操作" min-width="200" v-if="$check_action('/resume_information/table','set') || $check_action('/resume_information/view','set') || $check_action('/resume_information/view','get') 
				...
				<template slot-scope="scope">
					...
					v-if="$check_action('/resume_information/table','set') || $check_action('/resume_information/view','set') || $check_action('/resume_information/view','get')"
						...
						<!--跨表按钮-->
										<el-button class="e-button el-button--small is-plain el-button--primary" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/interview_notice/view')" v-if="($check_action('/interview_notice/table','add') || $check_action('/interview_notice/view','add')) && scope.row['examine_state'] == '已通过' && !scope.row['interview_notice_limit']">
						<span>邀请面试</span>
					</el-button>
											<el-button class="e-button el-button--small is-plain el-button--primary" style="margin: 5px !important;" size="small" @click="changeSigning(scope.row, scope.$index)" v-if="$check_option('/resume_information/table', 'examine')">
						<span>审核</span>
					</el-button>
				</div>
				</template>
			...
			get_list_before(param){
				...
				if(user_group != "管理员"){
						...
						sqlwhere+= "enterprise_users = " + this.user.user_id + " or ";
																		...
																		if(user_group=="注册用户"){
						sqlwhere+= "target_users = " + this.user.user_id + " or ";
					}
