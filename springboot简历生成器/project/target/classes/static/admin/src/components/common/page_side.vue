     ...
     <el-submenu index="user"
      ...
      <el-menu-item index="/user/table" v-show="user_group == '管理员'" @click="handleButtonClick(8)"><span>管理员</span></el-menu-item>
                ...
                <span>注册用户</span></el-menu-item>
                                              <el-menu-item index="/enterprise_users/table" v-show="$check_action('/enterprise_users/table')" @click="handleButtonClick(8)">
	...
	<el-submenu class="Cust" :index="i.id" v-for="i in auth" :key="i.id" v-show="$check_action(i.path)">
	  ...
	  <div v-else>
	    <el-submenu class="Cust" :index="'child'+ item.id" v-for="(item) in i.children" :key="item.id" v-show="$check_action(item.path)">
          ...
          <template slot="title"><i class="el-icon-warning-outline"></i><span ref="customTemplate2">公告通知管理</span>
          </template>
  ...
  export default {
    ...
    computed: {
      auth() {
        ...
        let site_list = []
		let map_list = []
                  		                    		                    		                    		                                site_list.push("resume_information")
          		                                site_list.push("interview_notice")
          		                  let list = ["exam", 'evaluation', "article", "article_type", "comment", "forum", "forum_type", "notice"];
