      ...
      <el-row class="rows row2">
         ...
         <el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
                ...
                <el-button @click="reset()" style="margin-right: 74px;" class="search_btn_reset">重置</el-button>
                                                      <el-button v-if="$check_action('/enterprise_users/table','add') || $check_action('/enterprise_users/view','add')" @click="$router.push('./view?')" class="add">添加</el-button>
            <el-button v-if="$check_action('/enterprise_users/table','del') || $check_action('/enterprise_users/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
                        </el-col>
	    ...
	    <el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
                  ...
                  <el-table-column prop="username" label="用户名" width="120">
                        ...
                        <el-table-column fixed prop="nickname" label="昵称">
                                              ...
                                              <el-table-column prop="enterprise_location" @sort-change="$sortChange" label="企业位置"                                v-if="$check_field('get','enterprise_location')" min-width="200">
                                </el-table-column>
      ...
      <el-table-column sortable prop="update_time" label="更新时间" min-width="200">
        <template slot-scope="scope">
          {{ $toTime(scope.row["update_time"],"yyyy-MM-dd hh:mm:ss") }}
                ...
                <el-table-column fixed="right" label="操作" min-width="120" v-if="$check_action('/enterprise_users/table','set') || $check_action('/enterprise_users/view','set') || $check_action('/enterprise_users/view','get')
					 ">
        

      <template slot-scope="scope">
        <router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
                     v-if="$check_action('/enterprise_users/table','set') || $check_action('/enterprise_users/view','set') || $check_action('/enterprise_users/view','get')"
                                 :to="'./view?user_id=' + scope.row['user_id']"
                                 size="small">
          <span>详情</span>
        </router-link>
                                                  		  		  		  	<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="changeSigning(scope.row, scope.$index)" v-if="$check_option('/enterprise_users/table', 'examine')">
		  		<span>审核</span>
		  	</el-button>
		        </template>
    </el-table-column>
        ...
        this.showModal = false;
			...
			assureVerify() {
				...
				examineForm.validate((valid) => {
					if (valid) {
						var url = "~/api/enterprise_users/set?enterprise_users_id=" + this.verifyItem['enterprise_users_id']
						_this.$post(url, {
